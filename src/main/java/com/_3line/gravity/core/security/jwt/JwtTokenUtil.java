...
//         Agents agent = agentRepository.findByUsername(username);
        if (agent==null) {
            return false;
        }
    ...
    public String getToken(String agentId) {
        final UserDetails userDetails = userDetailsService.loadUserByUsername(agentId);
        final String token = this.generateToken(userDetails, null);
        return token;
    }
